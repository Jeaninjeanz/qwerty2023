...
fData_categories <- fData_NA_rm_2 %>% 
  select( year, Name, risk1, risk2, risk3, risk4, risk5, risk6) %>% 
  group_by(Name) %>% 
...
```{r}
saveRDS(fData_categories_final, "~/Desktop/qwerty2023/data/cleaned/FundamentalDataCleaned.rds")
```
```{r}
fData2_categories2 <- fData_NA_rm_2 %>% 
  select( Date, Name, risk1, risk2, risk3, risk4, risk5, risk6) %>% 
  group_by(Name) %>% 
  mutate(category = (mean(c(risk1, risk2, risk3, risk4, risk5, risk6))))
